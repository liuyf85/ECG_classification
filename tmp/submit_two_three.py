...
def predict_labels(heartbeat_signals):
    ...
    model1 = BERT(input_size, hidden_size, 2, num_heads, num_layers, num_inter).to(device)
    model2 = BERT(input_size, hidden_size, 3, num_heads, num_layers, num_inter).to(device)
    model1.load_state_dict(torch.load('bert_two_class.pth'))
    model2.load_state_dict(torch.load('bert_three_class.pth'))
